...
// migrateDefaultPersonality adds the default_personality column to an existing
// group_configs table. New databases already have this column via initSchema;
// this function is a no-op for them. For existing databases it uses
// ALTER TABLE ADD COLUMN which is safe and atomic in SQLite.
func (s *Service) migrateDefaultPersonality() error {
	_, err := s.db.Exec(
		`ALTER TABLE group_configs ADD COLUMN default_personality TEXT NOT NULL DEFAULT 'resumobot'`)
	if err != nil {
		if strings.Contains(err.Error(), "duplicate column name") {
			return nil
		}
